...
class AndroidMediaPlayerCoordinator(DataUpdateCoordinator[dict[str, Any]]):
    ...
    Uses HA's DataUpdateCoordinator for push updates to ensure proper
    state propagation to all subscribers including Music Assistant.
    """
    ...
    async def async_disconnect(self) -> None:
        ...
        _LOGGER.info("Disconnecting from Android Media Player '%s'", self.device_name)
    ...
    async def _listen_websocket(self) -> None:
        ...
        _LOGGER.debug("Starting WebSocket message listener for '%s'", self.device_name)
        message_count = 0
        try:
            async for message in self._ws:
                ...
                try:
                    ...
                    old_state = (self.data or {}).get("state", "unknown")
                    new_state = data.get("state", "unknown")

                    if old_state != new_state:
                        _LOGGER.info(
                            "Android Media Player '%s' state changed: %s -> %s",
                            self.device_name, old_state, new_state
                        )
                    else:
                        _LOGGER.debug(
                            "Android Media Player '%s' state update: state=%s, title=%s",
                            self.device_name, new_state, data.get("mediaTitle")
                        )

                    # Use DataUpdateCoordinator's method to notify all listeners
                    self.async_set_updated_data(data)
                except json.JSONDecodeError as err:
                    _LOGGER.warning(
                        "Received invalid JSON from '%s': %s (message: %s)",
                        self.device_name, err, message[:100]
                    )
        except ConnectionClosed as err:
            _LOGGER.info(
                "WebSocket connection to '%s' closed: code=%s, reason=%s (received %d messages)",
                self.device_name, err.code, err.reason, message_count
            )
        except WebSocketException as err:
            _LOGGER.warning(
                "WebSocket error for '%s': %s (%s)",
                self.device_name, err, type(err).__name__
            )
        except asyncio.CancelledError:
            _LOGGER.debug("WebSocket listener for '%s' was cancelled", self.device_name)
            raise
        except Exception as err:
            _LOGGER.error(
    ...
    def _schedule_reconnect(self) -> None:
        ...
        if self._reconnect_task is None or self._reconnect_task.done():
            ...
            _LOGGER.debug(
                ...
                self._reconnect_count, self.device_name, next_interval
            )
    ...
    async def async_send_command(self, command: str, **kwargs: Any) -> bool:
        ...
        _LOGGER.debug(
            ...
            command, self.device_name, kwargs
        )
    ...
    async def _send_rest_command(self, command: str, **kwargs: Any) -> bool:
        ...
        if not endpoint:
            _LOGGER.warning("Unknown command '%s' for '%s'", command, self.device_name)
            return False
        ...
        try:
            async with aiohttp.ClientSession() as session:
                json_body = {}  # Default to empty object for commands without params
                ...
                async with session.post(
                    ...
                    if response.status == 200:
                        ...
                        _LOGGER.warning(
                            ...
                            command, self.device_name, response.status
                        )
                        return False
        except aiohttp.ClientConnectorError as err:
            _LOGGER.warning(
                "Cannot connect to '%s' for REST command '%s': %s",
                self.device_name, command, err
            )
            return False
        except asyncio.TimeoutError:
            _LOGGER.warning(
            ...
            _LOGGER.warning(
                ...
                self.device_name, command, err, type(err).__name__
            )
            return False
    ...
    async def async_get_state(self) -> dict[str, Any] | None:
        ...
        try:
                ...
                async with session.get(
                    ...
                    if response.status == 200:
                        ...
                        _LOGGER.debug(
                            ...
                            self.device_name, data.get("state")
                        )
                        self.async_set_updated_data(data)
                        return data
                    else:
                        _LOGGER.warning(
                            ...
                            self.device_name, response.status
                        )
        except aiohttp.ClientConnectorError as err:
            _LOGGER.warning(
            ...
            _LOGGER.warning("Timeout getting state from '%s'", self.device_name)
        except Exception as err:
            _LOGGER.warning(
